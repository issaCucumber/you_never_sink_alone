...
public class PortCannonActions : MonoBehaviour {
    ...
    public bool portcannonused = false;
    ...
    void Update () {
        ...
        if (portcannonused == false) {
            return;
        }

        // Traverse cannon
        transform.Rotate(0, 0, Input.GetAxis("Horizontal1") * Time.deltaTime * rotspeed * -1);
        ...
        if (Input.GetButtonDown("Fire1"))
        {
            ...
            if ((currenttime - lastfiretime) >= firerate) {
                Transform mycannonball = (Transform)Instantiate(portcannonball, GameObject.Find("PortCannonTip").transform.position, transform.rotation);
                mycannonball.GetComponent<Shoot>().level = portcannonpowerlevel;
